// ĐƯỜNG DẪN: src/pages/patient/booking/SelectTime.jsx
// THAY THẾ TOÀN BỘ NỘI DUNG FILE HIỆN TẠI
        ...
        <div className={styles.stepperContainer}>
          ...
          <div className={styles.stepLabels}>
            {['CHUYÊN KHOA', 'BÁC SĨ', 'THỜI GIAN', 'THÔNG TIN', 'XÁC NHẬN'].map((step, i) => (
              <div
                key={step}
                className={`${styles.stepLabel} ${i < 2 ? styles.stepLabelDone : i === 2 ? styles.stepLabelActive : ''
                  }`}
              >
                {step}
              </div>
                ...
                {loadingSlots ? (
                  ...
                  <div className={styles.timePeriodsList}>
                    {(() => {
                      ...
                      const periodsContent = TIME_PERIODS.map((period) => {
                        const Icon = period.icon;
                        
                        // Lọc slot theo buổi và thời gian hiện tại
                        const validSlots = availableSlots.filter(s => {
                          const [h, m] = s.time.split(':').map(Number);
                          
                          // Kiểm tra thuộc buổi này
                          if (!period.check(h)) return false;

                          // Nếu là hôm nay, chỉ hiện slot trong tương lai
                        ...
                        if (validSlots.length > 0) hasAnyValidSlot = true;
                        else return null;
                      ...
                      return hasAnyValidSlot ? periodsContent : (
                        <div className={styles.timeSlotsEmpty}>
                          <Moon size={24} opacity={0.5} />
                          <p>Bác sĩ không có lịch trống trong ngày này</p>
                        </div>
                      );
                    })()}
                  </div>
