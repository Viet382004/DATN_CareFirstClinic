...
import { Mail, Lock, User, Calendar, Eye, EyeOff, ArrowRight } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleRegister = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStep('verify');
      setSuccess('Đăng ký thành công. Mã xác thực (OTP) đã được gửi tới email của bạn!');
    } catch (error: any) {
  ...
  const handleVerifyOtp = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTimeout(() => navigate('/book', { replace: true }), 1500);
    } catch (error: any) {
      setError(error?.message || 'Xác thực OTP không thành công. Vui lòng kiểm tra lại.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setError(error?.message || 'Gửi lại OTP không thành công. Vui lòng thử lại.');
    }
  };
  ...
  return (
    <AuthLayout
      title={<>Đăng ký <br />tài khoản <span className="italic font-normal text-primary-light drop-shadow-none">CareFirst.</span></>}
      ...
      {step === 'register' ? (
        <form className="space-y-5" onSubmit={handleRegister}>
          ...
          <button
            ...
            className="w-full bg-primary hover:bg-primary-dark text-white font-semibold py-3 px-6 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-4 shadow-md"
          >
        ...
        <div className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-slate-600 mb-2">
              Chúng tôi đã gửi mã xác thực tới <strong>{formData.email}</strong>. Vui lòng kiểm tra hộp thư đến hoặc hộp thư rác (spam).
            </p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleVerifyOtp}>
            ...
            <button
              ...
              className="w-full bg-primary hover:bg-primary-dark text-white font-semibold py-3 px-6 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-md"
            >
          ...
          <div className="text-center space-y-4 pt-4 border-t border-slate-100">
            ...
            >
              {resendDisabled ? 'Vui lòng chờ (30s)' : 'Gửi lại mã OTP'}
            </button>
            <div className="pt-2">
              <button
                type="button"
